...
AAB::AAB(float minX, float minY, float minZ, float maxX, float maxY, float maxZ):
    points_{minX,minY,minZ,maxX,maxY,maxZ} {

}

AAB::AAB(float points[6]){
    for(int i=0;i<6;i++) {
        points_[i] = points[i];
    }
}

AAB::AAB() : points_{0,0,0,0,0,0} {

}

bool AAB::intersects(Ray ray, float& tVal){
...
AAB AAB::getTransformedPrimitiveAAB(glm::mat4 ctm){
    ...
    return AAB(points[0],points[1],points[2],points[3],points[4],points[5]);
}

float& AAB::operator[](int index) {
